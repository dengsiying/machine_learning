...
X = np.insert(X_raw,0,values=np.ones(X_raw.shape[0]),axis=1)
def expand_y(y):
...
theta = serialize(t1, t2) # 扁平化参数，25*401+10*26=10285 #对于一维数组拼接，axis的值不影响最后的结果
#feed forward（前向传播）
def sigmoid(z):
    return 1 / (1 + np.exp(-z))
def deserialize(seq):
...
def random_init(size):
    return np.random.uniform(-0.12, 0.12, size)#从一个均匀分布[low,high)中随机采样，注意定义域是左闭右开，即包含low，不包含high.
